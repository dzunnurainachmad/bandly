...
export default async function BrowsePage({ searchParams }: BrowsePageProps) {
  ...
  const chips: Chip[] = [
    activeProvince && {
      ...
      href: removeParam(sp, 'province', 'city'),
    },
    ...
    sp.open === 'true' && { label: t('openMemberChip'), href: removeParam(sp, 'open') },
  ...
  return (
    <FilterLoadingProvider>
      <div className="max-w-7xl mx-auto px-4 py-6 sm:py-8">
        <h1 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-stone-900 dark:text-stone-100">{t('title')}</h1>
            ...
            <ResultsOverlay>
              {bands.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <p className="text-lg font-medium text-stone-600 dark:text-stone-400">{t('notFound')}</p>
